...
for line in whole_file.split("All Info "):
    if line.find(" number =") != -1:
        counter += 1
        roll_id = line[0:9]
        place_nstrip = line.find('number =')
        if place_nstrip == -1: 
            print "No pattern found..."
            break
        nstrip = line[place_nstrip+8:place_nstrip+10]
        if roll_id in roll_id_nstrip.keys():
            print roll_id, " already in dict with entry ", roll_id_nstrip[roll_id]
        roll_id_nstrip[roll_id] = nstrip
...
print "Number of roll in json: ", len(roll_id_nstrip.keys())
print counter
with open('rollId_nstrip.json', 'w') as json_file:
      json.dump(roll_id_nstrip, json_file)
