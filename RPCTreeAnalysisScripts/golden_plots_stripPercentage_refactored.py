...
#file_list = ["/eos/cms/store/group/dpg_rpc/comm_rpc/Trigger/rpcNtuples/MinimumBias/MinimumBiasRun2018D-v1RAW-withRecHit-correct/200703_112929/0000/RPCTree_MinBias324344_digiMerger_withRecHit_137.root"]
    ...
    for event in rpc_tree:
        ...
        for digi_idx in range(event.digi_nDigi): #First loop to check if the strip was fired over consecutive BX's
            if str(event.digi_rawId[digi_idx]) in dict_roll_digistrip_BX:
                ...
                dict_roll_digistrip_BX[str(event.digi_rawId[digi_idx])] = {event.digi_strip[digi_idx] : [event.digi_bx[digi_idx]]}
        for roll in dict_roll_digistrip_BX.keys():
            for strip_idx in dict_roll_digistrip_BX[roll].keys():
                bxes = dict_roll_digistrip_BX[roll][strip_idx]
                if bxes.count(0) > 1 or -1 in bxes or not 0 in bxes:
                    continue
                if roll in dict_roll_numberOfDigi:
                    dict_roll_numberOfDigi[roll] += 1
                else:
                    dict_roll_numberOfDigi[roll] = 1
